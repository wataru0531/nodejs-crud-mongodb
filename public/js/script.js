...
async function showTask(){
  ...
  const { data: { allTasks } } = await axios.get(ENDPOINT);
  // console.log(allTasks); // (3) [{…}, {…}, {…}]
  
  if(allTasks.length === 0){ // タスクがない時の処理
...
function generateSingleTaskTemplate(_name, _memo, _id, _completed){
  const singleTask =  `
    <div class="task ${_completed ? "task-completed" : ""}">
      <div class="task__block">
        <div class="task__title">
          <span class="material-symbols-outlined check ${_completed ? "task-completed" : ""}">check_circle</span>
          <h2>${_name}</h2>
        </div>
        

        <div class="task__links task-links">

          <a href="./edit/index.html?id=${_id}" class="task-links__edit">
            <span class="material-symbols-outlined">edit</span>
          </a>

          <button class="task-links__delete delete-btn" data-id="${_id}">
            <span class="material-symbols-outlined">delete</span>
          </button>
        </div>
      </div>
      <div class="task__description">${_memo}</div>
    </div> 
  `;
